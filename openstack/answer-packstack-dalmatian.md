...
# Specify 'y' to install MariaDB database | 指定'y'安装MariaDB数据库 ['y', 'n']
CONFIG_MARIADB_INSTALL=y

# 全局默认密码 | Default password to be used everywhere (overridden by passwords set
CONFIG_DEFAULT_PASSWORD=

# 服务线程数 | The amount of service workers/threads to use for each service.
CONFIG_SERVICE_WORKERS=%{::processorcount}
...
# installation. This is helpful if you are running Packstack a second | 安装排除设置：当您需要重复运行Packstack并使用相同应答文件时，可在此处填写需要排除配置的服务器IP地址
...
# Specify 'y' if you want to run OpenStack services in debug mode; | 调试模式设置：指定'y'以在调试模式下运行OpenStack服务
# otherwise, specify 'n'. ['y', 'n']
CONFIG_DEBUG_MODE=n

# Server on which to install OpenStack services specific to the | 控制节点设置：在此指定安装OpenStack控制服务（如API服务器、仪表盘）的主机IP
# controller role (for example, API servers or dashboard).
CONFIG_CONTROLLER_HOST=192.168.0.101

# List the servers on which to install the Compute service. | 计算节点设置：在此列出安装计算服务的主机IP地址
CONFIG_COMPUTE_HOSTS=192.168.0.101

# List of servers on which to install the network service such as | 网络节点设置：指定安装网络服务（Nova网络或Neutron）的主机IP
# Compute networking (nova network) or OpenStack Networking (neutron).
CONFIG_NETWORK_HOSTS=192.168.0.101

# Specify 'y' if you want to use VMware vCenter as hypervisor and | VMware集成设置：指定'y'以使用VMware vCenter作为虚拟化管理程序
# storage; otherwise, specify 'n'. ['y', 'n']
CONFIG_VMWARE_BACKEND=n

# Specify 'y' if you want to use unsupported parameters. This should | 非支持参数设置：仅当您明确知晓风险时指定'y'使用非支持参数
...
# Specify 'y' if you want to use subnet addresses (in CIDR format) | 子网地址模式：指定'y'使用CIDR格式的子网地址替代接口名称配置
...
# specific to storage servers such as Image or Block Storage services. | （非支持！）存储节点设置：在此指定安装存储服务（如镜像或块存储服务）的主机IP
CONFIG_STORAGE_HOST=192.168.0.101
...
# also be specified).
CONFIG_MARIADB_HOST=192.168.0.101
...
# the database.
CONFIG_CINDER_DB_PW=5d87c45bc7ea4025
...
CONFIG_CINDER_KS_PW=003fe38f955c458f

# Storage backend to use for the Block Storage service; valid options
# are: lvm, nfs, vmdk, netapp, solidfire. ['lvm', 'nfs', 'vmdk',
# 'netapp', 'solidfire']
...
CONFIG_NEUTRON_KS_PW=592132c62472456b

# The password to use for OpenStack Networking to access the
# database.
CONFIG_NEUTRON_DB_PW=6b431dcf5d5b4bb9
...
CONFIG_NEUTRON_METADATA_PW=0fe3546922f540f6
...
# 'arista', 'mlnx', 'l2population', 'sriovnicswitch', 'ovn']
...
# ovs-bridge-mappings=ext-net:br-ex --os-neutron-ovs-bridge-
# interfaces=br-ex:eth0
CONFIG_NEUTRON_OVS_BRIDGE_IFACES=
...
# ex,physnet1:br-vlan" --os-neutron-ovs-bridge-interfaces="br-
# ex:eth1,br-vlan:eth2"
...
# be in the format <physical_network>:<ovs_bridge>. Example:
# physnet1:br-eth1,physnet2:br-eth2,physnet3:br-eth3
...
# the proper port. This can be achieved from the command line by
# issuing the following command: packstack --allinone --os-neutron-
# ovn-bridge-mappings=ext-net:br-ex --os-neutron-ovn-bridge-
# interfaces=br-ex:eth0
...
# ex,physnet1:br-vlan" --os-neutron-ovn-bridge-interfaces="br-
# ex:eth1,br-vlan:eth2"
...
CONFIG_HORIZON_SECRET_KEY=878c7348ceca4f1cbfe9c6bb9ccfebcc
...
# the Identity service.
CONFIG_SWIFT_KS_PW=5f5b4b9523af4edc
...
CONFIG_SWIFT_HASH=9f9421d06d254c55
...
CONFIG_HEAT_AUTH_ENC_KEY=620eb362dc04446c
...
# 数据库访问密码 | Password to use for Gnocchi to access the database.
CONFIG_GNOCCHI_DB_PW=733c69a5eef34bb7

# 身份认证密码 | Password to use for Gnocchi to authenticate with the Identity service.
CONFIG_GNOCCHI_KS_PW=eef2d0817dd04d9e

# 遥测服务密钥 | Secret key for signing Telemetry service (ceilometer) messages.
CONFIG_CEILOMETER_SECRET=eaca9e455d304e43

# Password to use for Telemetry to authenticate with the Identity
# service.
CONFIG_CEILOMETER_KS_PW=30319f7ed0004ea4
...
# Identity service.
CONFIG_AODH_KS_PW=c9f7851e9cfe406e

# Password to use for Telemetry Alarming (AODH) to access the
# database.
CONFIG_AODH_DB_PW=8e3521bd87604893
